...
## Pending native one-time key publication

The pending-device coordinator can now prepare and publish one batch of 100 native EC and 100 native PQ one-time keys for each ACI and PNI after the saved account/profile publication is acknowledged. It uses the existing native key ID allocators, generation/signing methods and `PreKey` SQLCipher stores. Private serialized records, allocation counters, canonical public request bytes and the enrollment journal commit atomically. The context hash binds the original attempt, account/device, membership binding, operation, credential, request metadata, entropy/profile publication and explicit origin/public authorities. Exact preparation repeats are read-only. Changed/consumed/foreign keys, allocation counters or account/profile/authority state reject; missing keys are never regenerated from an uncertain request.

Each identity uses fresh Basic authentication to the configured publication origin at `PUT /v2/keys?identity=aci` or `?identity=pni`, accepts only empty 204, and sends only `preKeys` and `pqPreKeys`. No signed or last-resort keys or identity replacement fields are uploaded by this action. Native PQ batches have their own 512 KiB public request bound; enrollment and response bounds remain 64 KiB. The existing restrictive transport rejects redirects and has no application retry/fallback loop.

The current server replaces a nonempty one-time pool and has no idempotency tombstone. **An uncertain dispatch cannot be replayed**, even by explicit user choice: replay could restore already-consumed key IDs. The client commits `dispatched` before HTTP and `acknowledged` after empty 204; cancellation, response loss or failed acknowledgement persistence leaves the identity blocked. ACI precedes PNI. Restart never clears that marker, acknowledged identities are skipped, and the setup UI offers no key-publication retry when uncertainty remains. Server idempotency/reconciliation is required before uncertain recovery can be enabled. This source does not claim transport-level exactly-once delivery.

Validation: the actual unsigned app/extensions/framework graph passed. The isolated exact-source host suite has 46 tests, covering native public request construction, original context, persistence-before-send, response contracts, restart after response/persistence failure, suspension and no repeated send after acknowledgement. The actual-framework SQLCipher/WAL file probe extends to 46 fresh-process phases, 18 verified SIGKILL boundaries and nine rollback-isolated prerequisite conflicts. Key generation, counters and journal roll back together; committed native private/public bytes survive restart unchanged. Synthetic acknowledgements in the probe exercise the local journal only, never a server. The standalone probe still does not instantiate the complete identity-manager/AppSetup graph, real Keychain/file protection, device power loss, enrollment or messaging.

No live publication, SMS, APNs or upstream service request is made. Explicit production endpoint/trust inputs, guarded profile live acceptance, safe key retry/idempotency, complete owned service and Groups/Stories/media routing, receiver authority/context verification, services-ready lifecycle/navigation and device acceptance remain release barriers. The normal-build and pending-services guards remain closed. No libsignal source/archive or generated Pods change belongs to this checkpoint.

## Owned initial-prekey publication journal

The next checkpoint supersedes the legacy route above for newly generated journals only. Version 2 freezes the canonical owned publication origin, authority/configuration hash, exact `/v1/bconnected/keys/initial/` route contract and separate ACI/PNI UUID operation IDs in the original context hash. These inputs commit with the native keys and exact public request bytes before dispatch. The body remains the same bounded `preKeys`/`pqPreKeys` JSON, sent with fresh Basic authentication at `PUT /v1/bconnected/keys/initial/{operationId}?identity=aci|pni`. Only empty 204 acknowledges an identity.

This dedicated backend contract records the first accepted operation and returns its result on identical replay without repopulating consumed keys. An explicit repeated setup action revalidates fresh enrollment status, saved account/authority/profile state and native keys, then resends the same unacknowledged operation. Reopening or repeating its SQLCipher dispatch marker performs no writes. An acknowledged ACI is skipped when PNI needs replay. There is no automatic HTTP retry, endpoint fallback, alternate operation ID or key regeneration after uncertainty. Missing routes and 400/401/404/409/503 responses preserve the unacknowledged marker. Backend deployment and live client/server acceptance remain separate prerequisites.

Version 1 journals decode without an added route and remain unchanged. Both prepared and dispatched legacy pools are blocked from publication or migration, including after restart; their keys may already belong to the old replace-pool endpoint. No reset/recovery flow is introduced.

Validation: the actual unsigned app/extensions/framework graph and 49 exact-source host tests pass. The standalone encrypted-file probe has 47 fresh-process phases, 18 deliberate SIGKILL boundaries, twelve rollback-isolated key/account/authority/profile/operation/endpoint conflicts and two legacy-journal cases. It verifies byte-identical owned replay after dispatch and acknowledgement crash boundaries, while preserving all private key records/counters and withholding account credentials, registration callbacks and readiness. Its HTTP acknowledgements are synthetic local state. The full identity-manager/AppSetup lifecycle, live server idempotency, Keychain/file protection, device messaging and Groups/Stories/receiver authority remain outside this checkpoint. Normal-build and pending-services guards stay closed; no production bundle inputs are added.

## Isolated owned group-manifest verifier and rollback memory

`BConnectedGroupManifest` consumes the frozen storage `bconnected/group-manifests.md` version-one contract: RFC 7515 compact JWS, RFC 9864 `Ed25519`, exact issuer/audience/type, canonical unpadded base64url and fixed schema-defined payload serialization. Platform CryptoKit verifies the original received header/payload segments. Construction requires one to eight explicit public Ed25519 JWK pins and per-pin current-response/history policy. Unknown/duplicate fields, alternate algorithms, embedded/discovered keys, private key fields, unsupported critical headers and malformed/oversized input reject. Compact splitting and native repeated-message preflight bound allocation before generic decoding. There are no production pins or network/key-discovery calls.

The verifier hashes exact envelope native bytes before parsing, validates derived group/public parameters, all access and announcement/termination fields, and rejects unknown protobuf fields recursively. It compares every active/invited raw member, role and declared principal one-for-one without the upstream parser's duplicate/invalid-invite normalization. Native libsignal decrypts every principal to the declared nonzero ACI using the expected group secret; PNI, malformed invites, duplicate principals/ACIs or clear-roster mismatch reject. Active profile keys also decrypt under that ACI. Only complete validation constructs the immutable verified snapshot.

`BConnectedGroupManifestStore` persists revision, exact native hash and canonical payload hash together in the caller's SQLCipher transaction. It rereads the rollback floor inside that transaction, rejects lower revisions or different hashes at equal revision, and performs no write for identical state. Key rotation preserves payload identity. Historical-purpose verification cannot update the current floor. The store is an integration component: future group-model/receiver persistence must join that same transaction after authenticated context checks; a signature or saved floor supplies no fresh admission/permission.

The standalone `group-manifest` probe uses the committed Java public-signing-key/synthetic-group fixture and real CryptoKit, native libsignal, generated protobuf and encrypted SQLCipher/WAL. It checks the Java signature/decrypted roster/exact hashes, signed clear-ACI divergence on first use and at a saved revision, explicit pin rotation/history, canonical/bounded parsing, native policy/profile/ACI/invited-member consistency, zero/max revisions and unknown native fields. Nine fresh processes and four deliberate SIGKILL boundaries verify atomic first/advanced floor rollback or recovery and zero-write repeats.

```sh
python3 Scripts/bconnected/probe_native_account_db.py --simulator <dedicated-simulator-UUID> --probe group-manifest
```

No existing GroupsV2 request, avatar fetch, message receiver, sender-key distribution, notification extension, display or registration consumer is enabled by this slice. Existing fetch code downloads avatars before its lossy parser, so a future owned route must perform the new strict verification before such effects. Authenticated outer context, fresh current group authority, queue/NSE/process replay, consented invitation bootstrap, directory/Stories/enqueue and 8000-recipient delivery remain separate. Native/manifest signatures cannot establish freshness or revoke already received content. Pending-services and normal-build guards remain closed, and no real group, production signing key, SMS/APNs or enrollment is used in the probe.

## Fresh pending account/profile readback

After account/profile publication and both version-two owned initial-prekey operations are acknowledged, an explicit setup action can verify the saved account and profile. It uses the same frozen `BConnectedAccountPublicationOrigin`, system trust and native-validated authority configuration; no production bundle values or inferred endpoint are introduced. Each invocation performs fresh enrollment status, authenticated `GET /v1/accounts/whoami`, then `GET /v1/profile/{savedACI}/{savedProfileVersion}`. The ephemeral transport requires bounded JSON, bypasses caches, cookies and stored credentials, rejects redirects, and has no application retry or upstream fallback. The ordinary account reader still withholds credentials.

The readback parser requires exact account ACI/PNI/phone and storage capability, then the saved ACI identity key, UAK checksum, visible capabilities and encrypted name/about/emoji/phone-sharing bytes. Missing requested versions fail because the required encrypted phone-sharing field must match. This first-signup slice rejects usernames, optional-phone salt, avatar, payment address and displayed badges; it never consumes account entitlements as grants. Duplicate/unknown response keys, malformed booleans/base64/identifiers, alternate status codes and changed fields reject. The two responses are independent observations, not an atomic remote snapshot or proof of future permission; the server must authorize each read separately. It does not read back the PNI identity key or prove remaining prekey counts.

The coordinator revalidates completed native keys, allocation counters, profile/UAK, self-recipient, entropy, original binding/account and configured authority in SQLCipher before status and around each response. Only the refreshed enrollment observation may change across the status call. Incomplete, uncertain or legacy key journals cannot enter native preparation or send readback requests. The database helper uses the validating repeat paths, compares the original journal before/after validation and queues no lifecycle callbacks. Missing/changed local material is never repaired by this action.

No acceptance receipt or readiness bit is persisted. A successful check only produces a transient setup message; restart and explicit recheck fetch fresh responses again. Uncertain account/profile publication still requires its existing explicit replay decision; readback does not reconcile it. Pending-services and normal-build guards remain closed.

Validation includes the actual unsigned app/extensions/framework graph and exact-source host tests. `SignalServiceKit/tests/Registration/Resources/bconnected-account-acceptance-v1.json` preserves exact Java `SystemMapper` response bytes from the real account identity builder and profile response builder, plus matching synthetic public inputs. Its encrypted field bytes are synthetic shape fixtures, not a profile-decryption test. The exact production parser accepts those bytes; existing native SQLCipher/profile tests retain their separate scope. The file-recovery probe adds read-only acceptance revalidation, stale snapshot rejection, and five rollback-isolated incomplete/legacy/missing-key/changed-profile conflicts; saved account credentials and readiness remain hidden.

Live admitted signup/publication/readback, complete native lifecycle, owned service/media configuration, and real-iPhone acceptance remain unverified. Group-model/manifest-floor atomic persistence, immutable authenticated receiver context through the durable queue and notification extension, consented invitation handling, Stories/announcements fanout, APNs and 8,000-recipient delivery remain required release gates. Later prekey replenishment remains a separate contract. This checkpoint does not enable any of those consumers.
